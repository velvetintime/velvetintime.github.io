document.addEventListener("DOMContentLoaded", () => {
  ...
  // =========================
  // FACHADA: 2 CLAVES + REGISTRO DE INTENTOS
  // =========================
  const CLAVE_ACCESO_LOCAL_FALLBACK = "2026secret";
  const CLAVE_SOLICITUD_LOCAL_FALLBACK = "2026pedir";

  const fachadaSecreta = document.getElementById("fachadaSecreta");
  const sutranWrapper = document.getElementById("sutranWrapper");
  const fachadaSecretInput = document.getElementById("fachadaSecretInput");
  const fachadaSecretButton = document.getElementById("fachadaSecretButton");
  const fachadaSecretMessage = document.getElementById("fachadaSecretMessage");
  const fachadaPanelAcceso = document.getElementById("fachadaPanelAcceso");
  const ACCESO_ADMIN_DIRECTO_URL = (() => {
    try {
      const valor = new URLSearchParams(window.location.search).get("collection") || "";
      return valor.trim().toLowerCase() === "blackvelvet";
    } catch (error) {
      return false;
    }
  })();

  let accesoDesdeFachada = false;
  let solicitudDesdeFachada = false;

  function mostrarMensajeFachada(texto, tipo = "info") {
    if (!fachadaSecretMessage) return;
    fachadaSecretMessage.textContent = texto || "";
    fachadaSecretMessage.classList.remove("fs-store-msg-error", "fs-store-msg-ok");
    if (tipo === "error") fachadaSecretMessage.classList.add("fs-store-msg-error");
    if (tipo === "ok") fachadaSecretMessage.classList.add("fs-store-msg-ok");
  }

  function esFormatoEmailValidoFachada(valor) {
    const texto = String(valor || "").trim();
    return /^[^\s@]+@[^\s@]+\.[^\s@]{2,}$/.test(texto);
  }

  function crearBotonVolverFachadaLogin() {
    if (!pantallaLogin) return;

    let botonVolver = document.getElementById("btnVolverFachadaDesdeLogin");
    if (!botonVolver) {
      botonVolver = document.createElement("button");
      botonVolver.id = "btnVolverFachadaDesdeLogin";
      botonVolver.type = "button";
      botonVolver.className = "btn-volver-fachada-login";
      botonVolver.textContent = "← Volver a la página";

      const loginBox = pantallaLogin.querySelector(".login-box");
      if (loginBox) {
        loginBox.insertBefore(botonVolver, loginBox.firstChild);
      }
    }

    botonVolver.style.display = (accesoDesdeFachada || solicitudDesdeFachada) && !ACCESO_ADMIN_DIRECTO_URL ? "inline-flex" : "none";
    botonVolver.onclick = volverAFachada;
  }

  function ocultarSolicitudEnLogin() {
    const loginSecundario = document.querySelector(".login-secundario");
    if (loginSecundario) {
      loginSecundario.style.display = accesoDesdeFachada ? "none" : "";
    }
  }

  function mostrarLoginAdminDirecto() {
    accesoDesdeFachada = true;
    solicitudDesdeFachada = false;

    if (fachadaSecreta) {
      fachadaSecreta.style.setProperty("display", "none", "important");
      fachadaSecreta.setAttribute("aria-hidden", "true");
    }

    if (fachadaPanelAcceso) {
      fachadaPanelAcceso.style.setProperty("display", "none", "important");
    }

    if (sutranWrapper) {
      sutranWrapper.style.setProperty("display", "block", "important");
      sutranWrapper.removeAttribute("aria-hidden");
    }

    document.body.classList.remove("fachada-activa");
    document.body.classList.add("sutran-activo");
    document.body.classList.add("fachada-login-acceso");
    document.body.classList.add("admin-directo-activo");
    document.documentElement.classList.add("admin-directo-activo");
    ...
    setTimeout(() => {
      crearBotonVolverFachadaLogin();
      ocultarSolicitudEnLogin();

      const botonVolver = document.getElementById("btnVolverFachadaDesdeLogin");
  ...
  function mostrarLoginSegunOrigen() {
    if (ACCESO_ADMIN_DIRECTO_URL) {
      mostrarLoginAdminDirecto();
      return;
    }
  ...
  function mostrarLoginDesdeFachada() {
    accesoDesdeFachada = true;
    solicitudDesdeFachada = false;
  ...
  function mostrarSolicitudDesdeFachada() {
    accesoDesdeFachada = false;
    solicitudDesdeFachada = true;
    ...
    document.body.classList.remove("fachada-activa");
    document.body.classList.add("sutran-activo");
    document.body.classList.add("fachada-solicitud-activa");
    ...
    setTimeout(() => {
      ...
      // Oculta visualmente el login por debajo del modal para que al tocar fuera
      // no quede expuesto el acceso al sistema.
      if (pantallaLogin) {
      ...
      if (typeof abrirModalSolicitud === "function") {
        abrirModalSolicitud();
      } else if (btnAbrirSolicitud) {
        btnAbrirSolicitud.click();
      }
    }, 80);
  }

  function volverAFachada() {
    accesoDesdeFachada = false;
    solicitudDesdeFachada = false;

    if (typeof cerrarModalSolicitud === "function") {
      try { cerrarModalSolicitud(); } catch (e) {}
    }
    ...
    if (sutranWrapper) {
      sutranWrapper.style.setProperty("display", "none", "important");
      sutranWrapper.setAttribute("aria-hidden", "true");
    }
    ...
    if (fachadaSecreta) {
      fachadaSecreta.style.setProperty("display", "block", "important");
      fachadaSecreta.removeAttribute("aria-hidden");
    }

    const loginSecundario = document.querySelector(".login-secundario");
    if (loginSecundario) loginSecundario.style.display = "";

    const botonVolver = document.getElementById("btnVolverFachadaDesdeLogin");
    if (botonVolver) botonVolver.style.display = "none";

    if (fachadaSecretInput) fachadaSecretInput.value = "";
    mostrarMensajeFachada("", "info");

    document.body.classList.add("fachada-activa");
    document.body.classList.remove("sutran-activo");
    document.body.classList.remove("fachada-login-acceso");
    document.body.classList.remove("fachada-solicitud-activa");

    window.scrollTo({ top: 0, behavior: "smooth" });
  }

  async function cargarConfigFachada() {
    try {
      const { data, error } = await supabase
        .from("fachada_config")
        .select("clave_acceso, clave_acceso_activa, clave_solicitud, clave_solicitud_activa")
        .eq("id", 1)
        .maybeSingle();

      if (error || !data) {
        console.warn("No se pudo cargar fachada_config. Usando respaldo:", error);
        return {
          clave_acceso: CLAVE_ACCESO_LOCAL_FALLBACK,
          clave_acceso_activa: true,
          clave_solicitud: CLAVE_SOLICITUD_LOCAL_FALLBACK,
          clave_solicitud_activa: true
        };
      }

      return data;
    } catch (error) {
      console.warn("Error general cargando fachada_config. Usando respaldo:", error);
      return {
        clave_acceso: CLAVE_ACCESO_LOCAL_FALLBACK,
        clave_acceso_activa: true,
        clave_solicitud: CLAVE_SOLICITUD_LOCAL_FALLBACK,
  ...
  async function registrarIntentoFachada(resultado, claveTipo, motivo) {
    try {
      const installationId = obtenerInstallationIdLocal ? obtenerInstallationIdLocal() : null;
      const deviceInfo = obtenerDeviceInfo ? obtenerDeviceInfo() : "";

      await supabase
        .from("fachada_intentos")
        .insert([{
          installation_id: installationId,
          device_info: deviceInfo,
          clave_tipo: claveTipo || "desconocida",
          resultado: resultado || "fallido",
          motivo: motivo || null
        }]);
    } catch (error) {
      console.warn("No se pudo registrar intento de fachada:", error);
    }
  }

  async function actualizarUltimoIntentoFachada(payload = {}) {
    try {
      const installationId = obtenerInstallationIdLocal ? obtenerInstallationIdLocal() : null;
      if (!installationId) return;

      const { data: ultimo, error: errorConsulta } = await supabase
        .from("fachada_intentos")
        .select("id")
      ...
      if (errorConsulta || !ultimo?.id) {
        if (errorConsulta) console.warn("No se pudo ubicar último intento de fachada:", errorConsulta);
        return;
      }
      ...
      if (errorUpdate) {
        console.warn("No se pudo actualizar trazabilidad de fachada:", errorUpdate);
      }
    } catch (error) {
      console.warn("Error vinculando trazabilidad de fachada:", error);
    }
  }

  async function marcarFachadaLlegoLogin() {
    await actualizarUltimoIntentoFachada({
      llego_login: true,
      llego_login_fecha: new Date().toISOString()
    });
  }

  async function marcarFachadaLoginFallido(usuarioIntentado, motivo) {
    await actualizarUltimoIntentoFachada({
      usuario_intentado: usuarioIntentado || "",
      login_fallido: true,
      login_fallido_fecha: new Date().toISOString(),
      motivo: motivo || "login_fallido"
    });
  }

  async function marcarFachadaLoginExitoso(usuarioFinal) {
    await actualizarUltimoIntentoFachada({
      usuario: usuarioFinal || "",
      usuario_intentado: usuarioFinal || "",
      login_exitoso: true,
  ...
  async function validarClaveFachadaSegura() {
    const valor = String(fachadaSecretInput?.value || "").trim();

    if (!valor) {
      mostrarMensajeFachada("Ingrese el correo para continuar.", "error");
      return;
    }

    if (fachadaSecretButton) fachadaSecretButton.disabled = true;

    try {
      const config = await cargarConfigFachada();

      if (config.clave_acceso_activa && valor === String(config.clave_acceso || "")) {
        mostrarMensajeFachada("Estableciendo conexión segura...", "info");
        await registrarIntentoFachada("exitoso", "acceso", "clave_acceso_correcta");
        await marcarFachadaLlegoLogin();
        mostrarMensajeFachada("", "info");
        if (fachadaSecretInput) fachadaSecretInput.value = "";
        mostrarLoginDesdeFachada();
        return;
      }

      if (config.clave_solicitud_activa && valor === String(config.clave_solicitud || "")) {
        mostrarMensajeFachada("Conectando con la compra...", "info");
        await registrarIntentoFachada("exitoso", "solicitud", "clave_solicitud_correcta");
        mostrarMensajeFachada("", "info");
        if (fachadaSecretInput) fachadaSecretInput.value = "";
        mostrarSolicitudDesdeFachada();
        return;
      }

      if (!esFormatoEmailValidoFachada(valor)) {
        await registrarIntentoFachada("fallido", "desconocida", "formato_email_invalido");
        mostrarMensajeFachada("Ingrese un correo válido para continuar.", "error");
        return;
      }

      await registrarIntentoFachada("fallido", "desconocida", "correo_sin_clave");
      mostrarMensajeFachada("Su correo está ingresando para continuar con la compra.", "ok");
      setTimeout(() => mostrarMensajeFachada("", "info"), 2200);
    } catch (error) {
      console.error("Error validando fachada:", error);
      await registrarIntentoFachada("fallido", "error", "error_validacion");
      mostrarMensajeFachada("No se pudo validar en este momento.", "error");
    } finally {
      if (fachadaSecretButton) fachadaSecretButton.disabled = false;
    }
  }

  if (fachadaSecretButton) {
    fachadaSecretButton.addEventListener("click", (e) => {
      e.preventDefault();
      validarClaveFachadaSegura();
    });
  }

  if (fachadaSecretInput) {
    fachadaSecretInput.addEventListener("keydown", (e) => {
      if (e.key === "Enter") {
        e.preventDefault();
        validarClaveFachadaSegura();
      }
  ...
  // Seguridad: si se abrió solo para solicitud, tocar fuera o cancelar
  // debe volver a la fachada, nunca dejar el login visible.
  [
    "cerrarModalSolicitudBackdrop",
    "btnCancelarSolicitud",
    "btnEntendidoSolicitud"
  ].forEach((id) => {
    const el = document.getElementById(id);
    if (!el) return;
    el.addEventListener("click", (event) => {
      if (!solicitudDesdeFachada) return;
  ...
  function aplicarAccesoAdminDirectoInicial() {
    if (!ACCESO_ADMIN_DIRECTO_URL) return;
    ...
    const fachada = document.getElementById("fachadaSecreta");
    const wrapper = document.getElementById("sutranWrapper");
    const login = document.getElementById("pantallaLogin");
    const app = document.getElementById("appReal");
    const panel = document.getElementById("fachadaPanelAcceso");

    if (fachada) {
      fachada.style.setProperty("display", "none", "important");
      fachada.setAttribute("aria-hidden", "true");
    }
  ...
  aplicarAccesoAdminDirectoInicial();
  ...
  const btnMenuFachada = document.getElementById("btnMenuFachada");
  const panelFachadaAdmin = document.getElementById("panelFachadaAdmin");
  const btnVolverMenuDesdeFachada = document.getElementById("btnVolverMenuDesdeFachada");
  const envolturaBotonVolverFachada = document.getElementById("envolturaBotonVolverFachada");
  const resumenFachadaAdmin = document.getElementById("resumenFachadaAdmin");
  const estadoClaveAccesoFachada = document.getElementById("estadoClaveAccesoFachada");
  const estadoClaveSolicitudFachada = document.getElementById("estadoClaveSolicitudFachada");
  const valorClaveAccesoFachada = document.getElementById("valorClaveAccesoFachada");
  const valorClaveSolicitudFachada = document.getElementById("valorClaveSolicitudFachada");
  const btnCambiarClaveAccesoFachada = document.getElementById("btnCambiarClaveAccesoFachada");
  const btnToggleClaveAccesoFachada = document.getElementById("btnToggleClaveAccesoFachada");
  const btnCambiarClaveSolicitudFachada = document.getElementById("btnCambiarClaveSolicitudFachada");
  const btnToggleClaveSolicitudFachada = document.getElementById("btnToggleClaveSolicitudFachada");
  const btnRecargarFachadaAdmin = document.getElementById("btnRecargarFachadaAdmin");
  const listaIntentosFachadaAdmin = document.getElementById("listaIntentosFachadaAdmin");
  const listaDispositivosFachadaAdmin = document.getElementById("listaDispositivosFachadaAdmin");
  ...
  let fachadaConfigAdminCache = null;
  let fachadaIntentosAdminCache = [];
  let seleccionIntentosFachadaAdmin = new Set();
  let seleccionActividadDetalleAdmin = new Set();
  ...
  function obtenerEstadoFachadaTexto(activo) {
    return activo ? "Activa" : "Inactiva";
  }

  function aplicarEstadoBadgeFachada(elemento, activo) {
    if (!elemento) return;
    elemento.textContent = obtenerEstadoFachadaTexto(activo);
    elemento.classList.remove("badge-fachada-activa", "badge-fachada-inactiva");
    elemento.classList.add(activo ? "badge-fachada-activa" : "badge-fachada-inactiva");
  }

  function resumirDeviceInfoFachada(info) {
    const texto = String(info || "").trim();
    if (!texto) return "Dispositivo no identificado";

    if (/android/i.test(texto)) {
      const match = texto.match(/Android\s+[^;)]*/i);
      return match ? match[0] : "Android";
    }

    if (/iphone|ipad/i.test(texto)) return "iPhone / iPad";
    if (/windows/i.test(texto)) return "Windows";
    if (/mac/i.test(texto)) return "Mac";
    return texto.slice(0, 70);
  }

  async function cargarFachadaAdmin() {
    if (!esAdminActual) {
      if (resumenFachadaAdmin) resumenFachadaAdmin.textContent = "Solo el admin puede ver este módulo";
      return;
    }

    if (resumenFachadaAdmin) resumenFachadaAdmin.textContent = "Cargando seguridad de fachada...";

    try {
      const [{ data: config, error: errorConfig }, { data: intentos, error: errorIntentos }] = await Promise.all([
        supabase
          .from("fachada_config")
          .select("clave_acceso, clave_acceso_activa, clave_solicitud, clave_solicitud_activa, actualizado_en")
          .eq("id", 1)
          .maybeSingle(),
        supabase
          .from("fachada_intentos")
.select("id, installation_id, device_info, clave_tipo, resultado, motivo, creado_en, usuario, usuario_intentado, llego_login, llego_login_fecha, login_exitoso, login_fecha, login_fallido, login_fallido_fecha")
          .order("creado_en", { ascending: false })
          .limit(30)
      ]);

      if (errorConfig) {
        console.error("Error cargando configuración de fachada:", errorConfig);
      }

      if (errorIntentos) {
        console.error("Error cargando intentos de fachada:", errorIntentos);
      }

      fachadaConfigAdminCache = config || {
        clave_acceso: "2026secret",
        clave_acceso_activa: true,
        clave_solicitud: "2026pedir",
        clave_solicitud_activa: true
      };

      fachadaIntentosAdminCache = intentos || [];
      seleccionIntentosFachadaAdmin.clear();

      renderizarFachadaAdmin();
    } catch (error) {
      console.error("Error general cargando fachada admin:", error);
      if (resumenFachadaAdmin) resumenFachadaAdmin.textContent = "No se pudo cargar la seguridad de fachada";
    }
  }

  function renderizarFachadaAdmin() {
    const config = fachadaConfigAdminCache || {};

    if (valorClaveAccesoFachada) valorClaveAccesoFachada.textContent = config.clave_acceso || "-";
    if (valorClaveSolicitudFachada) valorClaveSolicitudFachada.textContent = config.clave_solicitud || "-";

    aplicarEstadoBadgeFachada(estadoClaveAccesoFachada, !!config.clave_acceso_activa);
    aplicarEstadoBadgeFachada(estadoClaveSolicitudFachada, !!config.clave_solicitud_activa);

    const total = fachadaIntentosAdminCache.length;
    const exitosos = fachadaIntentosAdminCache.filter((i) => String(i.resultado || "").toLowerCase() === "exitoso").length;
    const fallidos = fachadaIntentosAdminCache.filter((i) => String(i.resultado || "").toLowerCase() === "fallido").length;

    if (resumenFachadaAdmin) {
      resumenFachadaAdmin.innerHTML = `
        <div><strong>${total}</strong><span>Intentos recientes</span></div>
        <div><strong>${exitosos}</strong><span>Exitosos</span></div>
        <div><strong>${fallidos}</strong><span>Fallidos</span></div>
      `;
    }

    renderizarIntentosFachadaAdmin();
    renderizarDispositivosFachadaAdmin();
  }

  function renderizarIntentosFachadaAdmin() {
    if (!listaIntentosFachadaAdmin) return;

    if (!fachadaIntentosAdminCache.length) {
      seleccionIntentosFachadaAdmin.clear();
      listaIntentosFachadaAdmin.innerHTML = "<p class='fachada-vacio'>No hay intentos registrados todavía.</p>";
      return;
    }

    const idsActuales = new Set(fachadaIntentosAdminCache.map((item) => String(item.id || "")).filter(Boolean));
    seleccionIntentosFachadaAdmin = new Set(
      Array.from(seleccionIntentosFachadaAdmin).filter((id) => idsActuales.has(id))
    );

    const todosSeleccionados = fachadaIntentosAdminCache.length > 0 &&
      fachadaIntentosAdminCache.every((item) => seleccionIntentosFachadaAdmin.has(String(item.id || "")));

    const toolbar = `
      <div class="toolbar-seleccion-fachada">
        <label class="check-admin-linea">
          <input type="checkbox" id="chkTodosIntentosFachada" ${todosSeleccionados ? "checked" : ""}>
          <span>Seleccionar todos</span>
        </label>
        <div class="toolbar-seleccion-acciones">
          <button id="btnEliminarIntentosSeleccionados" type="button">Eliminar seleccionados</button>
          <button id="btnBorrarTodosIntentosFachada" type="button">Borrar todos</button>
        </div>
      </div>
    `;

    const listaHtml = fachadaIntentosAdminCache.map((item) => {
      const idIntento = String(item.id || "");
      const resultado = String(item.resultado || "").toLowerCase();
      const esOk = resultado === "exitoso";
      const tipo = String(item.clave_tipo || "desconocida");
      const motivo = String(item.motivo || "-").replace(/_/g, " ");
      const fecha = item.creado_en ? formatearCreatedAt(item.creado_en) : "-";
      const dispositivo = resumirDeviceInfoFachada(item.device_info);
      const usuarioRelacionado = item.usuario || item.usuario_intentado || "";
      const estadoLogin = item.login_exitoso
        ? "Ingresó al sistema"
        : item.login_fallido
          ? "Intentó login y falló"
          : item.llego_login
            ? "Llegó al login"
            : "No llegó al login";
      const fechaLogin = item.login_fecha
        ? formatearCreatedAt(item.login_fecha)
        : item.login_fallido_fecha
          ? formatearCreatedAt(item.login_fallido_fecha)
          : item.llego_login_fecha
            ? formatearCreatedAt(item.llego_login_fecha)
            : "";

      return `
        <div class="item-intento-fachada ${esOk ? "intento-ok" : "intento-fallo"}">
          <label class="check-item-admin">
            <input type="checkbox" class="chk-intento-fachada" data-id="${escaparHtml(idIntento)}" ${seleccionIntentosFachadaAdmin.has(idIntento) ? "checked" : ""}>
          </label>
          <div class="item-intento-icono">${esOk ? "✓" : "!"}</div>
          <div class="item-intento-contenido">
            <div class="item-intento-titulo">${escaparHtml(tipo)} · ${escaparHtml(esOk ? "Exitoso" : "Fallido")}</div>
            <div class="item-intento-meta">${escaparHtml(fecha)}</div>
            <div class="item-intento-meta">${escaparHtml(dispositivo)}</div>
            <div class="item-intento-usuario">Usuario: ${escaparHtml(usuarioRelacionado || "Sin identificar")}</div>
            <div class="item-intento-estado">Estado: ${escaparHtml(estadoLogin)}${fechaLogin ? " · " + escaparHtml(fechaLogin) : ""}</div>
            <div class="item-intento-motivo">${escaparHtml(motivo)}</div>
          </div>
        </div>
      `;
    }).join("");

    listaIntentosFachadaAdmin.innerHTML = toolbar + listaHtml;
  }


  async function eliminarIntentosFachadaPorIds(ids) {
    const listaIds = (ids || []).filter(Boolean);

    if (!listaIds.length) {
      mostrarMensaje("Seleccione al menos un intento");
      return;
    }

    const confirmado = window.confirm(`¿Eliminar ${listaIds.length} intento(s) de fachada?`);
    ...
    if (error) {
      console.error("Error eliminando intentos de fachada:", error);
      mostrarMensaje("No se pudieron eliminar los intentos");
      return;
    }

    seleccionIntentosFachadaAdmin.clear();
    mostrarMensaje("Intentos eliminados");
    await cargarFachadaAdmin();
  }

  async function borrarTodosIntentosFachada() {
    const ids = fachadaIntentosAdminCache.map((item) => String(item.id || "")).filter(Boolean);

    if (!ids.length) {
      mostrarMensaje("No hay intentos para borrar");
      return;
    }

    const confirmado = window.confirm(`¿Borrar los ${ids.length} intentos visibles de fachada?`);
    if (!confirmado) return;
    ...
    if (error) {
      console.error("Error borrando intentos de fachada:", error);
      mostrarMensaje("No se pudieron borrar los intentos");
      return;
    }

    seleccionIntentosFachadaAdmin.clear();
    mostrarMensaje("Intentos visibles borrados");
    await cargarFachadaAdmin();
  }

  function renderizarDispositivosFachadaAdmin() {
    if (!listaDispositivosFachadaAdmin) return;

    const mapa = new Map();

    fachadaIntentosAdminCache.forEach((item) => {
      const id = item.installation_id || "sin_id";
      if (!mapa.has(id)) {
        mapa.set(id, {
          installation_id: id,
          ...
          exitosos: 0,
          fallidos: 0,
          ultimo: item.creado_en || null
        });
      }

      const row = mapa.get(id);
      row.total += 1;
      if (String(item.resultado || "").toLowerCase() === "exitoso") row.exitosos += 1;
      if (String(item.resultado || "").toLowerCase() === "fallido") row.fallidos += 1;
      if (item.creado_en && (!row.ultimo || new Date(item.creado_en) > new Date(row.ultimo))) {
        row.ultimo = item.creado_en;
      }
    });

    const dispositivos = Array.from(mapa.values());

    if (!dispositivos.length) {
      listaDispositivosFachadaAdmin.innerHTML = "<p class='fachada-vacio'>No hay dispositivos detectados todavía.</p>";
      return;
    }

    listaDispositivosFachadaAdmin.innerHTML = dispositivos.map((item) => {
      const estado = item.fallidos >= 3 ? "Observado" : item.exitosos > 0 ? "Detectado" : "Pendiente";
      const clase = item.fallidos >= 3 ? "dispositivo-observado" : item.exitosos > 0 ? "dispositivo-detectado" : "dispositivo-pendiente";
      const fecha = item.ultimo ? formatearCreatedAt(item.ultimo) : "-";

      return `
        <div class="item-dispositivo-fachada ${clase}">
          <div class="item-dispositivo-cabecera">
            <strong>${escaparHtml(resumirDeviceInfoFachada(item.device_info))}</strong>
            <span>${escaparHtml(estado)}</span>
          </div>
          <div class="item-dispositivo-meta">Último intento: ${escaparHtml(fecha)}</div>
          <div class="item-dispositivo-meta">Exitosos: ${item.exitosos} · Fallidos: ${item.fallidos}</div>
        </div>
      `;
    }).join("");
  }

  async function cambiarClaveFachada(tipo) {
    if (!esAdminActual) return;

    const campo = tipo === "acceso" ? "clave_acceso" : "clave_solicitud";
    const etiqueta = tipo === "acceso" ? "clave de acceso" : "clave de solicitud";
    const actual = fachadaConfigAdminCache?.[campo] || "";

    const nueva = window.prompt(`Ingrese nueva ${etiqueta}:`, actual);

    if (nueva === null) return;

    const valor = String(nueva || "").trim();

    if (valor.length < 4) {
      mostrarMensaje(`La ${etiqueta} debe tener al menos 4 caracteres`);
      return;
    }
    ...
    if (error) {
      console.error("Error cambiando clave de fachada:", error);
      mostrarMensaje("No se pudo cambiar la clave de fachada");
      return;
    }

    mostrarMensaje("Clave de fachada actualizada");
    await cargarFachadaAdmin();
  }

  async function toggleClaveFachada(tipo) {
    if (!esAdminActual) return;

    const campo = tipo === "acceso" ? "clave_acceso_activa" : "clave_solicitud_activa";
    const actual = !!fachadaConfigAdminCache?.[campo];

    const confirmado = window.confirm(actual ? "¿Desactivar esta clave?" : "¿Activar esta clave?");
    if (!confirmado) return;
    ...
    if (error) {
      console.error("Error cambiando estado de clave fachada:", error);
      mostrarMensaje("No se pudo actualizar el estado de la clave");
      return;
    }

    mostrarMensaje(actual ? "Clave desactivada" : "Clave activada");
    await cargarFachadaAdmin();
  }
  ...
  function ocultarTodosLosBotonesVolver() {
    [envolturaBotonVolverRegistro, envolturaBotonVolverHistorial, envolturaBotonVolverUsuarios, envolturaBotonVolverActividad, envolturaBotonVolverFachada, envolturaBotonVolverClaves].forEach((item) => {
      if (!item) return;
  ...
  function mostrarMenuAdmin() {
    ...
    if (panelFachadaAdmin) panelFachadaAdmin.style.display = "none";
    if (panelFachadaAdmin) panelFachadaAdmin.style.display = "none";
    if (panelFachadaAdmin) panelFachadaAdmin.style.display = "none";
    if (panelFachadaAdmin) panelFachadaAdmin.style.display = "none";
  ...
  function abrirSeccionAdmin(seccion) {
    ...
    if (panelFachadaAdmin) {
      panelFachadaAdmin.style.display = seccion === "fachada" ? "block" : "none";
    }
    ...
    if (seccion === "fachada" && envolturaBotonVolverFachada) {
      envolturaBotonVolverFachada.style.display = "block";
      envolturaBotonVolverFachada.style.pointerEvents = "auto";
    }
  ...
  async function revisarSesion() {
    try {
      ...
      await marcarFachadaLoginExitoso(perfil.usuario || user.email || "");
  ...
  async function iniciarSesion(e) {
    ...
    try {
      ...
      if (error || !data || !data.user) {
        ...
        await marcarFachadaLoginFallido(usuarioIngresado, "credenciales_incorrectas");
      ...
      if (!perfil) {
        await marcarFachadaLoginFallido(usuarioIngresado, "perfil_no_encontrado");
        if (mensajeLogin) mensajeLogin.textContent = "No se encontro el perfil del usuario";
        return;
      }
      ...
      if (perfil.activo !== true) {
        ...
        await marcarFachadaLoginFallido(usuarioIngresado, "usuario_inactivo");
        await supabase.auth.signOut();
        if (mensajeLogin) mensajeLogin.textContent = "Usuario inactivo o bloqueado";
        return;
      }
      ...
      if (!esAdminPerfil) {
        ...
        if (!validacionInstalacion.ok) {
          ...
          await marcarFachadaLoginFallido(usuarioIngresado, "dispositivo_no_autorizado");
          await supabase.auth.signOut();
          if (mensajeLogin) mensajeLogin.textContent = validacionInstalacion.mensaje || "No se pudo validar el dispositivo";
          return;
        }
      ...
      await marcarFachadaLoginExitoso(perfil.usuario || usuarioIngresado);
  ...
  if (listaActividadAdmin) {
    listaActividadAdmin.addEventListener("change", (e) => {
      const chkTodos = e.target.closest(".chk-todos-actividad-detalle");
      if (chkTodos) {
        const clave = chkTodos.getAttribute("data-clave") || "";
        const registros = historialActividadCache[clave] || [];
        seleccionActividadDetalleAdmin = new Set(
          chkTodos.checked
            ? registros.map((fila) => String(fila.id || "")).filter(Boolean)
            : []
        );
      ...
      const chk = e.target.closest(".chk-actividad-detalle");
      if (!chk) return;
      ...
      if (chk.checked) {
        seleccionActividadDetalleAdmin.add(id);
      } else {
        seleccionActividadDetalleAdmin.delete(id);
      }

      const clave = chk.getAttribute("data-clave") || detalleActividadExpandidoUserId || "";
      const chkTodosActual = listaActividadAdmin.querySelector(`.chk-todos-actividad-detalle[data-clave="${CSS.escape(clave)}"]`);
      const registros = historialActividadCache[clave] || [];
      if (chkTodosActual) {
        chkTodosActual.checked = registros.length > 0 &&
          registros.every((fila) => seleccionActividadDetalleAdmin.has(String(fila.id || "")));
      }
    });

    listaActividadAdmin.addEventListener("click", async (e) => {
      const btnEliminar = e.target.closest(".btn-eliminar-actividad-seleccionada");
      if (btnEliminar) {
        const clave = btnEliminar.getAttribute("data-clave") || detalleActividadExpandidoUserId;
        await eliminarActividadDetallePorIds(Array.from(seleccionActividadDetalleAdmin), clave);
        return;
      }

      const btnBorrarTodos = e.target.closest(".btn-borrar-toda-actividad");
      if (btnBorrarTodos) {
        const clave = btnBorrarTodos.getAttribute("data-clave") || detalleActividadExpandidoUserId;
        await borrarTodaActividadDetalle(clave);
      }
    });
  }
  ...
  if (btnMenuFachada) {
    btnMenuFachada.addEventListener("click", async () => {
      abrirSeccionAdmin("fachada");
      await cargarFachadaAdmin();
    });
  }
  ...
  if (btnVolverMenuDesdeFachada) {
    btnVolverMenuDesdeFachada.addEventListener("click", () => {
      mostrarMenuAdmin();
    });
  }
  ...
  if (listaIntentosFachadaAdmin) {
    listaIntentosFachadaAdmin.addEventListener("change", (e) => {
      const chkTodos = e.target.closest("#chkTodosIntentosFachada");
      if (chkTodos) {
        seleccionIntentosFachadaAdmin = new Set(
          chkTodos.checked
            ? fachadaIntentosAdminCache.map((item) => String(item.id || "")).filter(Boolean)
            : []
        );
        renderizarIntentosFachadaAdmin();
        return;
      ...
      const chk = e.target.closest(".chk-intento-fachada");
      if (!chk) return;
      ...
      if (chk.checked) {
        seleccionIntentosFachadaAdmin.add(id);
      } else {
        seleccionIntentosFachadaAdmin.delete(id);
      }

      const chkTodosActual = document.getElementById("chkTodosIntentosFachada");
      if (chkTodosActual) {
        chkTodosActual.checked = fachadaIntentosAdminCache.length > 0 &&
          fachadaIntentosAdminCache.every((item) => seleccionIntentosFachadaAdmin.has(String(item.id || "")));
      }
    });

    listaIntentosFachadaAdmin.addEventListener("click", async (e) => {
      const btnEliminar = e.target.closest("#btnEliminarIntentosSeleccionados");
      if (btnEliminar) {
        e.preventDefault();
        await eliminarIntentosFachadaPorIds(Array.from(seleccionIntentosFachadaAdmin));
        return;
      }

      const btnBorrarTodos = e.target.closest("#btnBorrarTodosIntentosFachada");
      if (btnBorrarTodos) {
        e.preventDefault();
        await borrarTodosIntentosFachada();
      }
    });
  }

  if (btnRecargarFachadaAdmin) {
    btnRecargarFachadaAdmin.addEventListener("click", async () => {
      await cargarFachadaAdmin();
    });
  }

  if (btnCambiarClaveAccesoFachada) {
    btnCambiarClaveAccesoFachada.addEventListener("click", async () => {
      await cambiarClaveFachada("acceso");
    });
  }

  if (btnToggleClaveAccesoFachada) {
    btnToggleClaveAccesoFachada.addEventListener("click", async () => {
      await toggleClaveFachada("acceso");
    });
  }

  if (btnCambiarClaveSolicitudFachada) {
    btnCambiarClaveSolicitudFachada.addEventListener("click", async () => {
      await cambiarClaveFachada("solicitud");
    });
  }

  if (btnToggleClaveSolicitudFachada) {
    btnToggleClaveSolicitudFachada.addEventListener("click", async () => {
      await toggleClaveFachada("solicitud");
    });
  }
  ...
  function dibujarLineaAPK(texto, x, y, escalaX = 1) {
    ...
    // Parámetros replicados del APK G10:
    // Paint.setTextSize(14), Paint.setShadowLayer(8, 0, 0, black), drawText(x=0).
    const ESCALA_Y = 1;
    ...
    // Ajuste fino de halo para igualar mejor el render del G10.apk:
    // mantiene tamaño/posición y solo suaviza el contorno/sombra.
  ...
  function dibujarLinea3APKPorTramos(texto, x, y, escalaBase = 1, escalaDesdeKm = 1) {
    const textoCompleto = String(texto || "");
    const indiceKm = textoCompleto.indexOf(" KM.");

    if (indiceKm < 0) {
      dibujarLineaAPK(textoCompleto, x, y, escalaBase);
      return;
    }

    const tramoInicial = textoCompleto.slice(0, indiceKm);
    const tramoDesdeKm = textoCompleto.slice(indiceKm);

    ctx.save();
    ctx.translate(Math.round(x), Math.round(y));
    configurarTextoAPK();
    ctx.scale(escalaBase, 1);
    ctx.fillText(tramoInicial, 0, 0);
    ctx.restore();

    const anchoInicial = medirTextoAPK(tramoInicial, escalaBase);

    ctx.save();
    ctx.translate(Math.round(x + anchoInicial), Math.round(y));
    configurarTextoAPK();
    ctx.scale(escalaDesdeKm, 1);
    ctx.fillText(tramoDesdeKm, 0, 0);
    ctx.restore();
  }
        ...
        if (!historialOrdenado.length) {
          ...
          detalleHtml = `
            <div class="actividad-admin-detalle">
              ${bloqueCabecera}
              <div class="toolbar-seleccion-actividad">
                <label class="check-admin-linea">
                  <input type="checkbox" class="chk-todos-actividad-detalle" data-clave="${escaparHtml(claveActividad)}" ${historialOrdenado.length > 0 && historialOrdenado.every((fila) => seleccionActividadDetalleAdmin.has(String(fila.id || ""))) ? "checked" : ""}>
                  <span>Seleccionar todos</span>
                </label>
                <div class="toolbar-seleccion-acciones">
                  <button type="button" class="btn-eliminar-actividad-seleccionada" data-clave="${escaparHtml(claveActividad)}">Eliminar seleccionados</button>
                  <button type="button" class="btn-borrar-toda-actividad" data-clave="${escaparHtml(claveActividad)}">Borrar todos</button>
                </div>
              </div>
                ...
                ${historialOrdenado.map((fila) => {
                  ...
                  return `
                    <div class="actividad-evento ${claseEvento}">
                      <label class="check-item-admin check-actividad-evento">
                        <input type="checkbox" class="chk-actividad-detalle" data-id="${escaparHtml(String(fila.id || ""))}" data-clave="${escaparHtml(claveActividad)}" ${seleccionActividadDetalleAdmin.has(String(fila.id || "")) ? "checked" : ""}>
                      </label>
  ...
  async function recalcularResumenActividadUsuario(claveActividad) {
    if (!claveActividad) return;

    const itemActividad = actividadAdminCache.find((item) => obtenerClaveActividad(item) === claveActividad) || null;
    if (!itemActividad) return;

    const userId = itemActividad.user_id || null;
    const usuario = itemActividad.usuario || "";

    try {
      let query = supabase
        .from("actividad_sistema_historial")
        .select("id, fecha_hora_ingreso, tipo");
      ...
      const { data, error } = await query.order("fecha_hora_ingreso", { ascending: false });

      if (error) {
        console.error("Error recalculando actividad:", error);
        return;
      }

      const registros = data || [];
      const ingresosExitosos = registros.filter((fila) => String(fila.tipo || "").toLowerCase() !== "fallido");
      const ultimoIngreso = ingresosExitosos[0]?.fecha_hora_ingreso || null;

      const updateData = {
        contador_ingresos: ingresosExitosos.length,
        ultima_conexion: ultimoIngreso
      };

      if (ultimoIngreso) {
        const fecha = new Date(ultimoIngreso);
        if (!Number.isNaN(fecha.getTime())) {
          ...
          updateData.fecha_ingreso = `${anio}-${mes}-${dia}`;
          updateData.hora_ingreso = `${hora}:${minuto}:${segundo}`;
        }
      } else {
        updateData.fecha_ingreso = null;
        updateData.hora_ingreso = null;
      }

      let updateQuery = supabase.from("actividad_sistema").update(updateData);

      if (userId) {
        updateQuery = updateQuery.eq("user_id", userId);
      } else {
        updateQuery = updateQuery.eq("usuario", usuario);
      }

      const { error: errorUpdate } = await updateQuery;

      if (errorUpdate) {
        console.error("Error actualizando resumen de actividad:", errorUpdate);
      }
    } catch (error) {
      console.error("Error general recalculando resumen de actividad:", error);
    }
  }

  async function eliminarActividadDetallePorIds(ids, claveActividad) {
    const listaIds = (ids || []).filter(Boolean);

    if (!listaIds.length) {
      mostrarMensaje("Seleccione al menos un registro de actividad");
      return;
    }

    const confirmado = window.confirm(`¿Eliminar ${listaIds.length} registro(s) de actividad?`);
    if (!confirmado) return;
    ...
    if (error) {
      console.error("Error eliminando actividad seleccionada:", error);
      mostrarMensaje("No se pudo eliminar la actividad seleccionada");
      return;
    }

    seleccionActividadDetalleAdmin.clear();
    if (claveActividad) {
      await recalcularResumenActividadUsuario(claveActividad);
  ...
  async function borrarTodaActividadDetalle(claveActividad) {
    if (!claveActividad) return;

    const registros = historialActividadCache[claveActividad] || [];
    const ids = registros.map((fila) => String(fila.id || "")).filter(Boolean);

    if (!ids.length) {
      mostrarMensaje("No hay actividad visible para borrar");
      return;
    }

    const confirmado = window.confirm(`¿Borrar los ${ids.length} registros visibles de actividad de este usuario?`);
    if (!confirmado) return;
    ...
    if (error) {
      console.error("Error borrando toda la actividad visible:", error);
      mostrarMensaje("No se pudo borrar la actividad");
      return;
    }
    ...
    await cargarHistorialActividadPorUsuario(claveActividad);
    await cargarActividadAdmin();
    mostrarMensaje("Actividad visible borrada");
  }
...
function formatearFechaHoraResumenAdmin() {
  ...
  async function enviarSolicitudAcceso() {
    ...
    if (!usuario || !clave || !nombreCompleto) {
      if (mensajeSolicitud) {
        mensajeSolicitud.textContent = "Complete correo, clave de acceso y nombres completos";
        mensajeSolicitud.classList.remove("mensaje-solicitud-exito");
    ...
    if (mensajeSolicitud) {
      mensajeSolicitud.textContent = solicitudDesdeFachada ? "Procesando activación..." : "Enviando solicitud...";
      mensajeSolicitud.classList.remove("mensaje-solicitud-exito");
      mensajeSolicitud.classList.remove("mensaje-solicitud-error");
    }
    ...
    try {
      ...
      if (mensajeSolicitud) {
        mensajeSolicitud.textContent = solicitudDesdeFachada ? "Solicitud enviada. La activación será revisada dentro de las 24 horas." : "Su solicitud sera revisada dentro de las 24 horas a partir de este envio. Luego pruebe ingresar nuevamente.";
        mensajeSolicitud.classList.remove("mensaje-solicitud-error");
    ...
    botonGenerar.addEventListener("click", function () {
      ...
      lector.onload = function (evento) {
        ...
        imagen.onload = function () {
          ...
          // Parámetros reales encontrados en el APK G10:
          // tamaño 14, X=0, Y: alto-40 / alto-25 / alto-10.
          // Fila 3: se mantiene igual hasta antes de KM y solo se abre mínimamente desde "KM." hacia la derecha.
          const ESCALA_X_LINEA_1 = 1;
          const ESCALA_X_LINEA_2 = 1;
          const ESCALA_X_LINEA_3 = 0.985;
          const ESCALA_X_LINEA_3_DESDE_KM = 0.995;

          const linea1 = "   CREADO POR:  SUTRAN";
          const linea2 = "   COORDENADAS: " + coordenadas;
          const linea3 = "   " + ubicacion + " - FECHA: " + fecha + " " + hora;
          ...
          // El APK dibuja primero la fila inferior, luego coordenadas y al final CREADO POR.
          dibujarLinea3APKPorTramos(linea3, x, y3, ESCALA_X_LINEA_3, ESCALA_X_LINEA_3_DESDE_KM);
